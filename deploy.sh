...
#
# This deploys the SLIM CPU webapp image (root Dockerfile, no cudaq) to ECS
# Express Mode. It always runs CPU simulation.
#
# GPU note (two-image model):
#   Real GPU acceleration lives in a SEPARATE on-demand image + stack:
#     - image:  Dockerfile.gpu  (repo pqc-demo-gpu, includes cudaq)
#     - infra:  infra/gpu-cfn.yml, driven by ./gpu-demo.sh
#   This script does NOT build or deploy the GPU image. See ./gpu-demo.sh.
#
# Usage:
#   ./deploy.sh
# ============================================
...
echo ""
echo "GPU: dipakai otomatis kalau task jalan di instance ber-GPU,"
echo "     kalau tidak, otomatis fallback ke CPU."
echo "================================"
